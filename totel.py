...
# def move_by_waypoints(waypoints):
...
#
#     # 设置一个参考距离，所有移动时间将基于这个距离进行缩放
#     reference_distance = 5
#     move_duration = 0.17
#
#     # 计算第一个点到第二个点的距离，作为参考移动时间的基准
#     first_distance = calculate_distance(waypoints[0], waypoints[1])
#     reference_duration = move_duration * (first_distance / reference_distance)
#
...
#
#         # 根据距离调整移动时间
#         duration = reference_duration * (distance / first_distance)
#
#         if next_point[0] > current_point[0]:  # 右移
#             move('s', duration)
#         elif next_point[0] < current_point[0]:  # 左移
#             move('w', duration)
#         elif next_point[1] > current_point[1]:  # 下移
#             move('d', duration)
#         elif next_point[1] < current_point[1]:  # 上移
#             move('a', duration)
#
#         # 为了确保移动到准确的位置，可以在每次移动后加入短暂的等待
#         time.sleep(0.1)
#         print("\n")
#     second_last_point = waypoints[-2]
#     last_point = waypoints[-1]
#     print("second_last_point", second_last_point)
#     print("last_point", last_point)
#     # 计算最后一个点到倒数第二个点的距离，作为参考移动时间的基准
#     distance = calculate_distance(second_last_point, last_point)
#     duration = reference_duration * (distance / first_distance)
#     if next_point[0] > current_point[0]:  # 右移
#         move('s', duration)
#     elif next_point[0] < current_point[0]:  # 左移
...
# def turn_near_to_0(centre_point, grid,proportion_x,proportion_y):
#     for i in range(-int(len(grid)*0.05), int(len(grid)*proportion_y)):
#         for j in range(-int(len(grid)*0.05), int(len(grid)*proportion_x)):
#             grid[centre_point[1] + i][centre_point[0] + j] = 0
#     return grid
...
def turn_near_to_0_renwu(centre_point, grid, proportion_x, proportion_y):
    ...
    for i in range(0, int(len(grid) * proportion_y) + 10):
        for j in range(0, int(len(grid) * proportion_x) + 10):
            if (abs(centre_point[0] + i - xxx) + abs(centre_point[1] + j - yyy)) < 24:
...
def tuya():
    region = (1540, 311, 1590, 361)
    matches = match_templates_and_store(region)
    print(matches)
    # 二值化
    img = screenshot()
    ...
    centre_point_1 = matches['renwu'][0]
    grid = turn_near_to_0(centre_point_1, grid
                          , proportion_x=0.5, proportion_y=0.5)
...
def xunlu():
    ...
    a = 0
    img = screenshot()
    region = (1479, 487, 1679, 687)
    img_map = img[region[1]:region[3], region[0]:region[2]]
    cv2.imshow("img_map", img_map)
    centre_point = (4, 4)  # 460  =  2 * 230 = 5 * 46
    width, height = 50, 50
    step_size = 4
    target_color1_range = ((120, 120, 120), (130, 130, 130))
...
while True:
    try:
        ...
        if path == None:
            grid = turn_near_to_0_renwu((centre_point[0] - 28, centre_point[1] - 28), grid, 0.2, 0.2)
            path = A星(start, goal, grid)

# print(path)
# for i in path:
#     pyautogui.moveTo(1493 + 150 + i[1] - centre_point[1], 87 + 150 + i[0] - centre_point[0])
...
#
#         region = (1349, 134, 1849, 601)
#         img_map = img[region[1]:region[3], region[0]:region[2]]
#         # cv2.imshow("img_map", img_map)
#         centre_point = (230, 230)
#         width, height = 230, 230
#         step_size = 2
#         grid = 三值化(img_map, step_size, width, (128, 128, 128),(204,204,204))
#
#         # #中心点周围9格的值变为0
#         # for i in range(-1, 2):
#         #     for j in range(-1, 2):
#         #         grid[centre_point[0] + i][centre_point[1] + j] = 1
#         #
#         # # 移动16格需要3.44秒
